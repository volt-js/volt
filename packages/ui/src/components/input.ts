...
/**
 * A text field: the label, the box, a line of help, and the message.
 *
 * ```html
 * <v-input :value="email" label="Email" name="email" type="email" required
 *          description="Only used to reply."></v-input>
 * ```
 *
 * The four parts are one component because they are one field. The ids that
 * tie the label to the box, and the box to its help and its error, come from
 * `createFormField`, along with the dirty and touched flags, the moment
 * validation is allowed to speak, and the platform's own verdict pushed back
 * into the control so the browser refuses a submit for the reason on screen.
 * The box is a real `<input>`: that is what submits, what `FormData` reads,
 * what `form.reset()` restores and what the browser validates.
 *
 ...
 *
 * What a caller writes on the tag reaches the `<input>`. It is the control,
 ...
 */
@Component({ selector: 'v-input', templateUrl: './input.html' })
export class VInput {
   ...
   * would never hear it.
   */
  /** Where it starts when the value is the input's own, and what a reset goes back to. */
  ...
  @Prop() name?: string;
  /** One of the single-line types. `number` is not among them — that is its own control. */
  @Prop() type: TextInputType = 'text';
  ...
  /** The on-screen keyboard to ask a phone for. */
  @Prop() inputMode?: string;
  /** Enforced by the platform, and reported as the platform words it. */
  @Prop() minLength?: number;
  /** Enforced by the platform. `input.remaining()` is what is left of it. */
   ...
   */
  readonly input: TextInput = createInput({
    ...
    ...(this.inputMode !== undefined ? { inputMode: this.inputMode } : {}),
   ...
   * each is trimmed and an empty one is read as unsaid, which is what the
   * description has always done and what the name should have done with it.
   */
  inputProps(): InputProps {
    const own = this.input.inputProps();
   ...
   */
  message(): string {
    return this.input.field.messages()[0] ?? '';
  }
}
